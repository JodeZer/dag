...
# 仅运行 100k 节点反序列化测试
go test -bench="BenchmarkUnmarshalJSON_100k" -benchmem -run=^$ -benchtime=5x
...
- 反序列化基准使用 `UnmarshalJSON` 函数，通过 `storableDAG` 实现
...
针对超大规模场景（10 万节点），我们测试了不同分叉数（3 叉、4 叉、5 叉）的平衡树在序列化、反序列化和查询操作方面的性能。

#### 1. 序列化性能

| 分叉数 | 节点数 | ns/op | ms/op | B/op | allocs/op | JSON 大小 |
|--------|--------|-------|-------|------|-----------|-----------|
| **3 叉树** | 100,000 | 115,697,433 | 115.70 | 110,318,734 | 733,680 | 11.42 MB |
| **4 叉树** | 100,000 | 101,805,242 | 101.81 | 91,818,208 | 700,337 | 11.14 MB |
| **5 叉树** | 100,000 | 98,175,667 | 98.18 | 82,268,256 | 680,332 | 11.18 MB |

**关键发现：**

1. **序列化耗时**：98-116 ms/操作，对于超大规模图来说性能表现良好
2. **内存分配**：约 82-110 MB，随分叉数增加略有降低
...
|--------|--------|-------|-------|------|-----------|
| **3 叉树** | 100,000 | 53,732,867 | 53.73 | 33,847,264 | 200,063 |
| **4 叉树** | 100,000 | 51,047,850 | 51.05 | 33,847,264 | 200,063 |
| **5 叉树** | 100,000 | 52,064,200 | 52.06 | 33,847,264 | 200,063 |

**关键发现：**

1. **反序列化耗时**：51-54 ms/操作，比序列化更快（约 0.5 倍）
2. **内存分配**：约 33.8 MB，约为序列化的 1/3
3. **分配次数**：20 万次，约为序列化的 1/3-1/4
4. **4 叉树性能最优**：反序列化时间最短
5. **反序列化性能优于序列化**：由于不需要 DFS 遍历，直接构建顶点和边，性能更优

#### 3. 查询性能对比（GetDescendants）

| 分叉数 | 节点数 | ns/op | ms/op | B/op | allocs/op |
|--------|--------|-------|-------|------|-----------|
| **3 叉树** | 100,000 | 38,714,600 | 38.71 | 27,052,756 | 38,614 |
| **4 叉树** | 100,000 | 37,358,925 | 37.36 | 22,994,169 | 28,827 |
| **5 叉树** | 100,000 | 32,016,825 | 32.02 | 22,118,808 | 23,289 |

**关键发现：**

1. **查询耗时**：32-39 ms/操作，性能随分叉数增加而提升
...
|------|--------|--------|--------|----------|
| **序列化时间** | 115.70 ms | 101.81 ms | 98.18 ms | 5 叉树 |
| **序列化内存** | 110.3 MB | 91.8 MB | 82.3 MB | 5 叉树 |
| **反序列化时间** | 53.73 ms | 51.05 ms | 52.06 ms | 4 叉树 |
| **反序列化内存** | 33.8 MB | 33.8 MB | 33.8 MB | 所有相同 |
| **查询时间** | 38.71 ms | 37.36 ms | 32.02 ms | 5 叉树 |
| **查询内存** | 27.1 MB | 23.0 MB | 22.1 MB | 5 叉树 |
| **JSON 大小** | 11.42 MB | 11.14 MB | 11.18 MB | 4 叉树 |

**性能分析：**

- **5 叉树**：在序列化、查询性能和内存分配上都最优
- **4 叉树**：反序列化时间最短，JSON 大小最小
- **3 叉树**：整体性能略逊，但在某些特定场景可能更适合
- **反序列化 vs 序列化**：反序列化约为序列化时间的 0.5 倍，内存约为 1/3，性能显著更优

#### 4. 规模扩展性对比

| 规模 | 图类型 | 序列化 (ms) | 反序列化 (ms) | 查询 (ms) |
|------|--------|-------------|---------------|-----------|
| 1,111 节点 | 宽树 (4 层 × 10 分支) | ~1-2 | ~0.7 | ~0.1 |
| 100,000 节点 | 平衡树 (5 分叉最优) | ~98 | ~52 | ~32 |

**扩展性分析：**

1. **序列化扩展性**：从 1.1k 到 100k 节点，序列化时间增长约 50-100 倍（接近线性）
2. **反序列化扩展性**：从 1.1k 到 100k 节点，反序列化时间增长约 70 倍（接近线性）
3. **查询扩展性**：GetDescendants 性能随规模增长，但在 10 万节点规模下仍保持在可接受范围
4. **反序列化/序列化比例**：约 0.5-0.6 倍，反序列化始终比序列化更快
...
- **可行性强**：10 万节点序列化约 95 ms，性能表现良好
- **存储需求**：JSON 大小约 11 MB，相对紧凑
- **建议分叉数**：5 叉树（最佳反序列化性能）或 4 叉树（最佳序列化时间）

#### 2. 反序列化场景

- **可行性强**：10 万节点反序列化约 50 ms，性能优于序列化
- **内存效率高**：内存分配约 34 MB，约为序列化的 1/3
- **建议分叉数**：5 叉树（最佳反序列化性能）
